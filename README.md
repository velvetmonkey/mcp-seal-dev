...
> Scope note: the box describes the family runtime (the deployed `seal-host` boundary). This repo is the proof layer that boundary cites — the kernel verdict and the v2 canonical approval tuple — not the deployment itself; the demo sidecar here is a separate, demo-only path.
